...
public class LocalFileServerPlugin: CAPPlugin {
    ...
    private func startServer() {
        ...
        server.addHandler(forMethod: "GET", pathRegex: "/file", request: GCDWebServerRequest.self) { [weak self] request in
            ...
            if request.hasByteRange {
                response = GCDWebServerFileResponse(file: path, byteRange: request.byteRange)
            } else {
                response = GCDWebServerFileResponse(file: path)
            }
    ...
    @objc func refreshFolderAccess(_ call: CAPPluginCall) {
        refreshAccess()
        call.resolve(["success": accessedURL != nil])
    }
